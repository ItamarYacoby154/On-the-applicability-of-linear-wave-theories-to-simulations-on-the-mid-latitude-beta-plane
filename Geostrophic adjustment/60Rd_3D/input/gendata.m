...
ny = 3600;    % number of gridpoints in y-direction
xo = -0.1;     % origin in x,y for ocean domain
yo = -0.5;     % (i.e. southwestern corner of ocean domain)
dx = 0.05;    % grid spacing in x (km)
dy = 0.5;    % grid spacing in y (km)
...
theta_const = 20;
theta = (theta_const-4.6) * ones(nx, ny, nr);
theta(:, [1:1800], [1:15]) = theta_const + 4.6;
theta(:, [1801:end], [1:23]) = theta_const + 4.6;

fid = fopen('theta0.bin', 'w', ieee);
fwrite(fid, theta, accuracy);
fclose(fid);
